...
//        }
    ...
    void SAG_APD::layout(int l) {
        double ration = deviceL/deviceW+2.0,viagap = 0.0_um, metalgap = 0.5e-6, ppadl = 0.3,contrrolangle  =1;
        double ridgeshift = 0,laserTaper = 0_um, laserout = 2_um, lasertaperout = 2_um, pcontact = 0.6, pvia =0.7, nholegap = 1_um, nviagap = 0.15_um;
        double padwidth = 40_um, padgap = 15_um, nmetalmiddle = 30_um, nmetalmiddlegap = 3_um, thedta = 4,lno = 0;
        double laserRidge = 4_um,nanogap = 5_um;
        double pcontactgap = 1_um, ncontactgap = 5_um;
           BLOCK
           {
            add_O_Port("in0", cp, WG_Structure_List["waveguide1"]);
            layer(L_waveguides);
...
//        triangle(lno,lasertaperout,0,lasertaperout);
    ...
    ////Pcontact
            layer(L_pmetal);
            double pviaM2 = 0.85;
            move(pcontactgap,ridgeshift,0,NOFLIP);
            sw(deviceL-2*pcontactgap,laserRidge-2*pcontactgap);
        };
    ...
    ////Ncontact
            layer(L_nmetal);
            double Nmetal =1.1;
            move(pcontactgap,laserRidge/2+ncontactgap,0,NOFLIP);
            sw(deviceL-2*pcontactgap,laserRidge-2*pcontactgap);
        };
